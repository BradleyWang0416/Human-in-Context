    ...
    def forward(self, x, **kwargs):
        ...
        if self.mode == 'temporal':
            ...
            if kwargs.get('input_mask', None) is not None:
                spatial_mask = kwargs['input_mask']    # (B,J)
                spatial_adj_mask = torch.einsum('bik,bkj->bij', spatial_mask.unsqueeze(-1), spatial_mask.unsqueeze(-2))    # (B,J,J)
                for jj in range(j): spatial_adj_mask[:, jj, jj] = 1
                spatial_adj_mask = spatial_adj_mask.unsqueeze(1).expand(-1,t,-1,-1)     # (B,T,J,J)
                adj = spatial_adj_mask.reshape(b*t, j, j)
            else:
                adj = self.adj
                adj = self.change_adj_device_to_cuda(adj)
