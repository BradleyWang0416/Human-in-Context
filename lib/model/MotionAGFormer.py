...
class STMamba(nn.Module):
    ...
    def forward_spatial(self, x):
        B, T, J, C = x.shape
        x = x.reshape(B * T, J, C)  # (B x T, J, C)
        x = self.mamba(x)   # (B x T, J, C)
        x = x.reshape(B, T, J, C)   # (B, T, J, C)
        return x  # (B, T, J, C)

    def forward_temporal(self, x):
        B, T, J, C = x.shape
        x = x.permute(0, 2, 1, 3).reshape(B * J, T, C)  # (B x J, T, C)
        x = self.mamba(x)   # (B x J, T, C)
        x = x.reshape(B, J, T, C).permute(0, 2, 1, 3)   # (B, J, T, C)
        return x  # (B, J, T, C)


class AGMBlock(nn.Module):
    """
    Implementation of AGM block.
    """
    ...
    def __init__(self, dim, mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop=0., drop_path=0.,
                 ...
                 temporal_connection_len=1, neighbour_num=4, n_frames=243, num_joints=17, connections=None):
        super().__init__()
        ...
        if mixer_type == 'attention':
            ...
            self.mixer = STMamba(dim=dim, mode=mode)
        else:
            raise NotImplementedError("AGM mixer_type is either attention or graph or mamba")
    ...
    def forward(self, x, **kwargs):
        ...
        if self.use_layer_scale:
            x = x + self.drop_path(
                ...
                * self.mixer(self.norm1(x), **kwargs))
            x = x + self.drop_path(
                self.layer_scale_2.unsqueeze(0).unsqueeze(0)
                * self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.mixer(self.norm1(x), **kwargs))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x


class MotionAGMBlock(nn.Module):
    """
    ...
    def __init__(self, dim, mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop=0., drop_path=0.,
                 ...
                 num_joints=17, connections=None, **kwargs):
        super().__init__()
        self.hierarchical = hierarchical
        dim = dim // 2 if hierarchical else dim

        # ST Attention branch
        self.att_spatial = AGMBlock(dim, mlp_ratio, act_layer, attn_drop, drop, drop_path, num_heads, qkv_bias,
                                         ...
                                         connections=connections)
        self.att_temporal = AGMBlock(dim, mlp_ratio, act_layer, attn_drop, drop, drop_path, num_heads, qkv_bias,
                                          qk_scale, use_layer_scale, layer_scale_init_value,
        ...
        if graph_only:
            ...
            if use_tcn:
                ...
                self.graph_temporal = GCN(dim, dim,
                                          ...
                                          temporal_connection_len=temporal_connection_len,
                                          connections=connections)
        else:   # default for all 26 layers for MotionAGFormer-large.
            self.graph_spatial = AGMBlock(dim, mlp_ratio, act_layer, attn_drop, drop, drop_path, num_heads,
                                               qkv_bias,
                                               qk_scale, use_layer_scale, layer_scale_init_value,
                                               ...
                                               num_joints=num_joints,
                                               connections=connections)
            self.graph_temporal = AGMBlock(dim, mlp_ratio, act_layer, attn_drop, drop, drop_path, num_heads,
                                                qkv_bias,
                                                qk_scale, use_layer_scale, layer_scale_init_value,
        ...
        # ST Mamba branch
        self.mamba_spatial = AGMBlock(dim, mlp_ratio, act_layer, attn_drop, drop, drop_path, num_heads, qkv_bias,
                                         qk_scale, use_layer_scale, layer_scale_init_value,
                                        mode='spatial', mixer_type="mamba")
        self.mamba_temporal = AGMBlock(dim, mlp_ratio, act_layer, attn_drop, drop, drop_path, num_heads, qkv_bias,
                                         qk_scale, use_layer_scale, layer_scale_init_value,
                                        mode='temporal', mixer_type="mamba")

        self.use_adaptive_fusion = use_adaptive_fusion  # True for all 26 layers for MotionAGFormer-large.
        if self.use_adaptive_fusion:
            self.fusion = nn.Linear(dim * 3, 3)
            self._init_fusion()
    ...
    def forward(self, x, **kwargs):
        ...
        if self.hierarchical:
            ...
            x_attn = self.att_temporal(self.att_spatial(x_attn, **kwargs), **kwargs)
            x_graph = self.graph_temporal(self.graph_spatial(x_graph + x_attn, **kwargs), **kwargs)
        else:
            x_attn = self.att_temporal(self.att_spatial(x, **kwargs), **kwargs)
            x_graph = self.graph_temporal(self.graph_spatial(x, **kwargs), **kwargs)

            x_mamba = self.mamba_temporal(self.mamba_spatial(x, **kwargs), **kwargs)
            if np.any(np.isnan(x_mamba.detach().cpu().numpy())):
                raise ValueError("x_mamba nan.")
        ...
        if self.hierarchical:
            ...
            x = x_attn * alpha[..., 0:1] + x_graph * alpha[..., 1:2] + x_mamba * alpha[..., 2:3]
        else:
            x = (x_attn + x_graph + x_mamba) * 0.333
...
def create_layers(dim, n_layers, mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop_rate=0., drop_path_rate=0.,
    ...
    for _ in range(n_layers):
        layers.append(MotionAGMBlock(dim=dim,
                                          mlp_ratio=mlp_ratio,
                                          act_layer=act_layer,
                                          attn_drop=attn_drop,
                                          drop=drop_rate,
...
# Motion Attention-GCNFormer-Mamba
class MotionAGM(nn.Module):
    """
    ...
    def __init__(self, n_layers, dim_in, dim_feat, dim_rep=512, dim_out=3, mlp_ratio=4, act_layer=nn.GELU, attn_drop=0.,
                 ...
                 neighbour_num=4, n_frames=243, connections=None, **kwargs):
        """
    ...
    def forward(self, x, return_rep=False, **kwargs):
        """
