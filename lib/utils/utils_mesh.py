...
def compute_smpl_vertex(args, data_dict, SMPL_MODEL=None, info_dict=None):
...
def evaluate_mesh(results, eval_verts=True):
    if eval_verts:
    ...
    if eval_verts:
        ...
        mpve = np.mean(np.sqrt(np.square(pred_verts - target_verts).sum(axis=2)), axis=1)
    ...
    error_dict = {
        # 'mpve': mpve.mean(),                    # mpve: (N,)
        'mpjpe': mpjpe.mean(),                  # mpjpe: (N,)
        'pa_mpjpe': pa_mpjpe.mean(),            # pa_mpjpe: (N,)
        'mpjpe_17j': mpjpe_17j.mean(),          # mpjpe_17j: (N,)
        'pa_mpjpe_17j': pa_mpjpe_17j.mean(),    # pa_mpjpe_17j: (N,)
        }
    if eval_verts:
        error_dict['mpve'] = mpve.mean()
    return error_dict
